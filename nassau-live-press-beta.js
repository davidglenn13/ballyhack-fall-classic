...
(() => {
  ...
  function enhanceScore(){
    ...
    const existingStatus=app.querySelector('.nlp-match-status');
    ...
    if(!cards.length){
      scoreCard?.insertAdjacentHTML('afterend',statusHtml);
      const statusCard=app.querySelector('.nlp-match-status');
      moveSelectedPicker(r,g,statusCard||scoreCard);
      return;
    }
    const pressHtml=`<section class="card nlp-card"><div class="eyebrow">PRESS WAGERS</div><h2>Press Wagers</h2><p class="muted">Each press is a separate wager from its starting hole through the end of ${seg.label}.</p><div class="nlp-wagers">${cards.join('')}</div>${!c.value?'<p class="notice" data-nlp-hint>Set Wager Amount in the Side Game selection above to activate Press.</p>':''}</section>`;
    scoreCard?.insertAdjacentHTML('afterend',statusHtml+pressHtml);
  ...
  document.addEventListener('click',e=>{
    ...
    if(original){
      ...
      const teamsNow=nassauTeams(roundGroupNames(r,g),seg.pairing),counterTeamNow=original.pressedBy==='a'?teamsNow[1]:teamsNow[0];
      if(!counterTeamNow.includes(currentUser())){alert('Only the opposing team may Press the Press.');render();return;}
    ...
    const teamsNow=nassauTeams(roundGroupNames(r,g),seg.pairing),pressingTeam=loser==='a'?teamsNow[0]:teamsNow[1];
...
+c.value:'not entered'}</small></div><div class="nlp-side"><span>Current Match</span><b>${viewedStanding.loser?`${shortTeam(viewedStanding.loser==='a'?teams[0]:teams[1])} ${viewedStanding.margin} down`:'All square'}</b></div><div class="nlp-side"><span>New Bet Starts</span><b>Hole ${nextHole}</b></div><button type="button" class="primary" data-nlp-add data-r="${r}" data-g="${g}" data-si="${si}" data-hole="${nextHole}" ${c.value&&canPress?'':'disabled'} title="${canPress?'':'Only '+shortTeam(losingTeam||[])+' can press'}">Press</button></div>`);
    }
    const statusHtml=`<section class="card nlp-match-status"><div class="eyebrow">LIVE NASSAU · ${fmt.replace('Nassau ','')}</div><h2>Match Status</h2><div class="nlp-overall"><span>Current Standing</span><b>${overallMatchText(viewedStanding,teams)}</b></div></section>`;
    const scoreCard=scoreSide.closest('.card')||app.querySelector('.card');
    if(!cards.length){
