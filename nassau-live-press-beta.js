/* Add Nassau wager and manual press entry directly to the live Score page. Only the team currently down may press, only on the next unplayed hole, and only once per Nassau segment. */
(() => {
  ...
  function latestPressInSegment(c,si){
    return c.presses.filter(p=>+p.segment===si).sort((a,b)=>(+b.fromHole)-(+a.fromHole))[0]||null;
  }
  ...
  function removeSideGameWagerInputs(){
    document.querySelectorAll('.side-result-panel .nb-controls').forEach(x=>{ const label=x.querySelector('label'); if(label)label.remove(); });
  }
  ...
  function enhanceScore(){
    ...
    const segmentPresses=c.presses.filter(p=>+p.segment===si), activePress=latestPressInSegment(c,si);
    const pressRecorded=!!activePress;
    ...
    let status='';
    if(pressRecorded){
      status=`A press has been recorded in this match and started on Hole ${activePress.fromHole}.`;
    }else if(isFirstHole){
  ...
  document.addEventListener('change',e=>{
    const w=e.target.closest?.('[data-nlp-wager]'); if(!w)return;
    const r=+w.dataset.r,g=+w.dataset.g,c=cfg(r,g); c.value=Math.min(999,Math.max(0,parseInt(w.value,10)||0)); w.value=c.value||''; persist(r,g).then(()=>render());
  });

  document.addEventListener('click',e=>{
    const b=e.target.closest?.('[data-nlp-add]'); if(!b)return;
    const r=+b.dataset.r,g=+b.dataset.g,si=+b.dataset.si,h=+b.dataset.hole,c=cfg(r,g),seg=NASSAU_SEGMENTS[si],nextHole=nextUnplayedHole(r,g,seg);
    sanitizePresses(r,g,si,seg,c,nextHole);
