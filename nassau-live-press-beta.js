...
(() => {
  ...
  function enhanceScore(){
    ...
    const existingWager=document.querySelector('.side-game-picker .nlp-wager');
    if(existingCard){
      if(existingWager&&typeof sideGameWasSelected==='function'&&sideGameWasSelected(r,g)){existingWager.classList.add('side-game-wager-detached');existingWager.querySelector('.wager-next')?.replaceChildren('Wager Set');existingCard.insertAdjacentElement('afterend',existingWager);}
      return;
    }
