...
const HANDICAPS={'David Glenn':14,'Nick Condeni':6,'Jason Wain':18,'Joe Phelan':17,'Tyler Bohannon':15,'Scott Karl':10,'Bill McCombs':16,'Will Long':12};
...
export function validateWagerChange({names,format,scores,previous,next,actor,admin=false}){
  ...
  if(Number(old.value)!==next.value&&hasScores(names,scores)&&!admin)return 'The wager amount cannot change after scoring begins';
  if(oldPresses.length&&Number(old.value)!==next.value)return 'Remove existing presses before changing the original wager';
  ...
  for(const p of next.presses){
    ...
    if(oldP){
      if(JSON.stringify(oldP)!==JSON.stringify(p))return 'Recorded presses cannot be edited';
      continue;
    }
    ...
    if(!admin&&!teamsFor(names,seg.pairing)[p.pressedBy==='a'?0:1].includes(actor))return 'Only the pressing team may place this wager';
  }
  for(const p of next.presses){
