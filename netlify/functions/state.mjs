...
export default async (req) => {
  ...
  try {
    ...
    if (op === 'score') {
      ...
      await db.sql`INSERT INTO tournament_players (player,last_accessed_at,updated_at) VALUES (${body.player},NOW(),NOW()) ON CONFLICT (player) DO UPDATE SET last_accessed_at=NOW(),updated_at=NOW()`;
    } else if (op === 'charge') {
