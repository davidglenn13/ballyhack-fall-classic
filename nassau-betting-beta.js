...
(() => {
  ...
  window.nassauCashNetAll=totalNet;
  ...
  function enhanceSettlement(){
    ...
    card.insertAdjacentHTML('afterend',`<section class="card nb-settlement"><div class="eyebrow">CASH SIDE BETS</div><h2>Nassau Settlement</h2><p>Both Nassau formats and their elected presses are netted across all rounds. In 5-5-5-3, holes 16–18 are three separate full-wager one-hole matches. This stays separate from tournament entry and Ballyhack trip charges.</p><div class="nb-net">${PLAYERS.map(p=>{const v=net[p.name]||0;return `<div><span>${p.name}</span><strong>${v>0?'+':''}${money(v)}</strong></div>`}).join('')}</div><div class="nb-payment-panel"><h3>Who Pays Who</h3>${pay.length?pay.map(x=>`<div class="nb-pay"><b>${x.from}</b><span>pays</span><b>${x.to}</b><strong>${money(x.amount)}</strong></div>`).join(''):`<p class="notice">${any?'No payment is due from completed results yet.':'Enter Nassau wagers and results will populate here automatically.'}</p>`}</div></section>`);
  }

  function style(){if(document.querySelector('#nb-style'))return;const s=document.createElement('style');s.id='nb-style';s.textContent=`.nb-controls{display:grid;grid-template-columns:1fr;gap:12px;align-items:end;margin-bottom:10px}.nb-controls>div{display:grid;gap:4px;padding:9px 11px;border-radius:10px;background:rgba(23,54,93,.07)}.nb-controls small{font-weight:700}.nb-box{margin-top:9px;padding-top:8px;border-top:1px solid var(--line)}.nb-box summary{cursor:pointer;font-size:12px;font-weight:900}.nb-no-press{margin-top:9px;padding-top:8px;border-top:1px solid var(--line);font-size:11px;color:var(--muted)}.nb-press{display:grid;grid-template-columns:1fr 1fr auto;gap:7px;align-items:center;padding:7px 0;border-bottom:1px solid var(--line)}.nb-press>div{display:grid;gap:2px}.nb-press small{font-size:10px;color:var(--muted)}.nb-press button{border:0;background:transparent;font-size:22px}.nb-net{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:7px;margin:12px 0 18px}.nb-net>div{display:flex;justify-content:space-between;gap:8px;padding:9px;border:1px solid var(--line);border-radius:9px}.nb-payment-panel{margin-top:22px;padding:16px;border:1px solid rgba(23,54,93,.20);border-left:5px solid var(--navy);border-radius:12px;background:rgba(23,54,93,.045);box-shadow:0 5px 16px rgba(23,54,93,.06)}.nb-payment-panel h3{margin:0 0 11px;color:var(--navy)}.nb-pay{display:grid;grid-template-columns:1fr auto 1fr auto;gap:8px;margin-top:7px;padding:11px 12px;border:1px solid var(--line);border-radius:9px;background:#fff;align-items:center}.nb-pay span{font-size:12px;color:var(--muted)}@media(max-width:650px){.nb-net{grid-template-columns:1fr}.nb-pay{grid-template-columns:1fr auto 1fr}.nb-pay strong{grid-column:1/-1;text-align:right}}`;document.head.appendChild(s)}
